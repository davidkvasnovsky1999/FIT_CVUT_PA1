                                                              ...
                                                              FlightsContainer          * pFlightsContainer )
{
    ...
    {
        pFlightsContainer->numberOfAllocatedCharsForNames *= 2;
                                                              ...
                                                              Flight                    * currentFlight )
{
    ...
    do
    {
        ...
        {
            currentFlight->name = pFlightsContainer->names + pFlightsContainer->namesLength;
...
int                     initFlightsContainer                ( FlightsContainer         ** ppFlightsContainer )
{
    ...
    *ppFlightsContainer                                               = ( FlightsContainer * ) malloc ( sizeof ( FlightsContainer ) );
    ...
    if ( ( (**ppFlightsContainer).flights                             = ( Flight * )              malloc ( ( **ppFlightsContainer ).numberOfAllocatedFlights               * sizeof ( Flight ) ) )
      ...
      && ( (**ppFlightsContainer).indexesOfClosestPairs               = ( unsigned long long * )  malloc ( ( **ppFlightsContainer ).numberOfAllocatedIndexesOfClosestPairs * sizeof ( unsigned long long ) ) ) )
        return 1;
    freeFlightsContainer ( ppFlightsContainer );
    return 0;
}
...
{
    int ret = 1;
    if ( checkFlightsContainerIntegrity ( ppFlightsContainer ) < 0 )
        return -1;
    if ( ! ( ( **ppFlightsContainer ).flights
...
int                     getFlights                          ( FlightsContainer         ** ppFlightsContainer )
{
    if ( ppFlightsContainer == NULL )
        return 0;
    initFlightsContainer ( ppFlightsContainer );
    int ret = 0;
    long double x, y;
...
int                     printPairsOfFlightsWithSmallestDist ( FlightsContainer          * pFlightsContainer )
{
    ...
    {
        printf ( "%llu %llu\n", *( pFlightsContainer->indexesOfClosestPairs + i ), *( pFlightsContainer->indexesOfClosestPairs + i + 1 ) );
        printf ( "%s - %s\n",
                 ( ( *pFlightsContainer ).flights + *( pFlightsContainer->indexesOfClosestPairs + i ) )->name,
                 ( ( *pFlightsContainer ).flights + *( pFlightsContainer->indexesOfClosestPairs + i + 1 ) )->name );
    }
    return 1;
}
...
int                     main                                ( void )
{
    ...
    if ( getFlights ( &pFlightsContainer ) )
    {
        ...
        printFlights ( pFlightsContainer );
//        printFlightNames ( pFlightsContainer );
